...
class WildberriesClient:
    ...
    def get_campaigns(self, *, type_filter: int = 0, limit: int = 1000, offset: int = 0) -> List[Dict[str, Any]]:
        """
        Fetch campaign list via WB /adv/v1/promotion/count endpoint.
        """
        try:
            resp = self.get("/adv/v1/promotion/count", timeout=self.timeout)
            ...
            logger.error("WB /adv/v1/promotion/count request failed: %s", exc)
            return []

        data = resp.json()
        adverts = data.get("adverts") if isinstance(data, dict) else None
        if not isinstance(adverts, list):
            logger.warning("WB /adv/v1/promotion/count unexpected payload: %s", data)
            return []
